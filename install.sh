...
if ! conda env list | awk '{print $1}' | grep -qx "$ENV_NAME"; then
  conda create --solver libmamba -y -n "$ENV_NAME" -c conda-forge -c bioconda \
    snakemake python=3.11 pandas biopython scikit-learn matplotlib networkx hmmer blast mmseqs2 cmake make
fi
